        ...
        return `
            <div class="day-column">
                ...
                <div class="tasks-container-semanal">
                    ${tasks.map(x => `
                        <div class="card-semanal" style="background: var(--color-${x.k==='Ex'?'ex':(x.k==='Rev'?'rev':'e')}); ${x.c ? 'opacity:0.5' : ''}">
                            <b style="font-size:0.6rem;">${x.m}</b><br>
...
function impEdital() { const m = document.getElementById('add-mat').value.toUpperCase(); const txt = document.getElementById('add-ass').value; if(!m || !txt.trim()) return; txt.split('\n').filter(l => l.trim().length > 1).forEach(a => { db.lista.push({ m, a: a.trim(), h: {E:1.5, Rev:1.0, Ex:1.0}, done: {E:false, Rev:false, Ex:false}, hF: 0 }); }); db.metaFixa = {}; save(); alert("Matéria Integrada!"); }
